    ...
    <main className="max-w-7xl mx-auto px-6 pt-24 pb-16 relative z-10">
      ...
      >
        <motion.div variants={fadeUp} transition={{ duration: 0.7 }}>
          <p className="text-sm font-medium tracking-[0.2em] uppercase text-blue-400/80 mb-4">
          ...
          <motion.p
            className="text-lg text-white/40 max-w-xl leading-relaxed"
            variants={fadeUp}
      ...
      >
        ...
        <GlassCard className="bento-span-2 bento-row-2 p-0" delay={0} onClick={onGoToChat}>
          <motion.div
            className="relative h-full p-8 flex flex-col justify-between overflow-hidden"
          ...
          >
            ...
            <div className="relative space-y-4">
              <div className="inline-flex items-center gap-2 text-blue-400 text-sm font-medium tracking-wide">
                <motion.span
              ...
              <h2 className="text-3xl font-bold tracking-tight text-white">Open chat</h2>
        ...
        {[
          ...
          <GlassCard key={stat.label} className="p-6" delay={0.1 + idx * 0.1}>
            ...
            <p className="text-xs text-white/30 mt-1 mb-3 uppercase tracking-wider font-medium">{stat.label}</p>
        ...
        {[
          ...
          <GlassCard key={card.title} className="p-6" delay={0.3 + idx * 0.1}>
            <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${card.gradient} flex items-center justify-center mb-4`}>
              <card.icon className={`w-5 h-5 ${card.iconColor}`} />
            </div>
            <h3 className="font-semibold text-white/90 mb-1 tracking-tight">{card.title}</h3>
      ...
      >
        <motion.div className="flex items-center justify-between mb-6" variants={fadeUp} transition={{ duration: 0.5 }}>
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-semibold tracking-tight text-white">Recent chats</h2>
          </div>
          <motion.button
            type="button"
            onClick={onGoToChat}
            ...
            whileTap={{ scale: 0.95 }}
            className="text-xs font-medium text-blue-400/70 hover:text-blue-400 transition-colors tracking-wider uppercase"
          >
            View all
          </motion.button>
        </motion.div>
        ...
        {loading ? (
          ...
          <GlassCard className="p-12 text-center">
            <p className="text-sm text-white/30">No conversations yet. Open chat to get started.</p>
          </GlassCard>
        ) : (
            ...
            <AnimatePresence>
              {conversations.map((c, i) => (
                <GlassCard key={c.id} delay={i * 0.06} onClick={() => onOpenConversation(c.id)} className="p-5">
                  <p className="font-medium text-white/80 truncate text-sm tracking-tight">
                    {c.title}
                  </p>
        ...
        >
          <GlassCard className="p-8" hover={false}>
            <div className="flex items-center gap-3 mb-1">
              ...
              <h3 className="font-semibold text-white tracking-tight">Policies per payer</h3>
            </div>
            ...
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  />
                  <Bar dataKey="policies" radius={[6, 6, 0, 0]} animationDuration={1400}>
      ...
      >
        <motion.div variants={fadeUp} transition={{ duration: 0.5 }}>
          <div className="flex items-center gap-3 mb-2">
            ...
            <h2 className="text-xl font-semibold tracking-tight text-white">Drug × payer coverage</h2>
          </div>
          <p className="text-sm text-white/30 max-w-3xl leading-relaxed">
        ...
        {crossRefLoading ? (
          ...
          <>
                ...
                <table className="w-full text-sm border-collapse min-w-[640px]">
                  <thead>
                    <tr className="border-b border-white/[0.04] bg-white/[0.02]">
                      <th className="sticky left-0 z-20 bg-[#0a0a14] px-4 py-3 text-left font-semibold text-white/50 border-r border-white/[0.04] min-w-[12rem] text-xs uppercase tracking-wider">
                        Drug
                      </th>
                      {crossRef.payers.map((payer) => (
                        <th
                          key={payer}
                          className="px-3 py-3 text-center font-semibold text-white/50 whitespace-nowrap max-w-[10rem] text-xs uppercase tracking-wider"
                          title={payer}
                        >
                  ...
                  <tbody>
                    {crossRef.drugs.map((drug, i) => (
                      ...
                      >
                        <td className="sticky left-0 z-10 bg-[#0a0a14] px-4 py-2.5 font-medium text-white/70 border-r border-white/[0.04] align-top text-sm">
                          {drug}
                        </td>
                        {crossRef.payers.map((payer, j) => {
            ...
            <GlassCard className="overflow-hidden" hover={false}>
              <motion.button
                ...
                className="w-full px-6 py-4 border-b border-white/[0.04] bg-white/[0.02] flex items-center justify-between select-none"
              >
                <div>
                  <h3 className="text-sm font-semibold text-white/80 tracking-tight">Policy list</h3>
                  ...
                  >
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="text-left text-white/30 border-b border-white/[0.04]">
                        ...
                        <tbody>
                          {crossRef.entries.map((e, idx) => (
                            ...
                            >
                              <td className="px-4 py-2.5 text-white/70">{e.drug_name}</td>
                              <td className="px-4 py-2.5 text-white/40">{e.payer_name}</td>
                              <td className="px-4 py-2.5 text-white/40 font-mono text-xs">{e.hcpcs_code ?? "—"}</td>
          ...
          <motion.p variants={fadeUp} transition={{ duration: 0.4 }} className="text-sm text-white/25">
