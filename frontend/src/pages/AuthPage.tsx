...
export function AuthPage({ onSignIn, onSignUp }: AuthPageProps) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-black px-4 relative overflow-hidden noise-overlay">
      <Suspense fallback={null}>
        <SceneBackground />
      </Suspense>

      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,rgba(0,0,255,0.06)_0%,transparent_60%)]" />
      ...
      >
        ...
        >
          ...
          <form ref={formRef} onSubmit={handleSubmit} className="space-y-5">
            <div>
              <label className="block text-sm font-medium text-white/50 mb-1.5 tracking-wide">
                Email
              </label>
              <motion.div
                animate={{
                  borderColor: focusedField === "email" ? "rgba(59,130,246,0.5)" : "rgba(255,255,255,0.06)",
              ...
              >
                <input
                  ...
                  className="w-full px-4 py-3 bg-white/[0.03] border border-white/[0.06] rounded-xl text-sm text-white placeholder-white/20 focus:outline-none transition-colors"
            ...
            <div>
              ...
              <motion.div
                animate={{
                  borderColor: focusedField === "password" ? "rgba(59,130,246,0.5)" : "rgba(255,255,255,0.06)",
            ...
            <AnimatePresence>
              {error && (
                <motion.p
                  ...
                  className="text-sm text-red-400 bg-red-500/10 border border-red-500/15 px-3 py-2 rounded-lg"
                >
                  {error}
                </motion.p>
              )}
              {message && (
                <motion.p
                  ...
                  animate={{ opacity: 1, height: "auto" }}
                  exit={{ opacity: 0, height: 0 }}
                  className="text-sm text-emerald-400 bg-emerald-500/10 border border-emerald-500/15 px-3 py-2 rounded-lg"
                >
                  {message}
                </motion.p>
              )}
